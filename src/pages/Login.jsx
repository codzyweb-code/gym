...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('s4fitness_token', data.token)
      localStorage.setItem('s4fitness_user', JSON.stringify(data.user))
      ...
      <div className="container" style={{ maxWidth: 440 }}>
        <div className="glass-card" style={{ padding: 'var(--space-2xl)' }}>
          ...
          <p style={{ textAlign: 'center', marginTop: 'var(--space-xl)', fontSize: 'var(--fs-sm)', color: 'var(--color-text-muted)' }}>
            ...
            <Link to="/register" className="text-accent" style={{ fontWeight: 'var(--fw-semibold)' }}>Sign Up</Link>
          </p>
        </div>
      </div>
    </div>
